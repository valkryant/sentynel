...
do
    function Utility:Log(Type, Message)
        ...
        if Type == 'error' then
            error('[ sen ] Error: ' .. Message)
        elseif Type == 'log' then
            print('[ sen ] ' .. Message)
        elseif Type == 'warn' then
            warn('[ sen ] Warning: ' .. Message)
        end
    end
    ...
    function Utility:EnableDragging(Frame, Parent)
        ...
        local Main = CoreGui:FindFirstChild('sentynel| .gg/ZJdWF6RmeM').Main
    ...
    function Utility:Destroy()
        ...
        for _, UI in next, CoreGui:GetChildren() do
            if UI.Name == 'sentynel| .gg/ZJdWF6RmeM' then
...
function Library:CreateWindow(Properties)
    -- // Properties
    local Name = Utility:GetProperty('Window', 'Name', Properties) or 'sentynel'
    local IntroText = Utility:GetProperty('Window', 'IntroText', Properties) or 'sentynel'
    local IntroIcon = Utility:GetProperty('Window', 'IntroIcon', Properties) or 'rbxassetid://128211042613950'
    ...
    local function Execute(String)
        ...
        local Command = assert(Commands[First], '[sen] Command Not Found: ' .. First)
